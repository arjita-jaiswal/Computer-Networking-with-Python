...
PrintDrop()
{
	std::cout <<"time : "<< Simulator::Now().GetSeconds() << "\t" <<"Mac TxDrop : "<< MacTxDropCount << "\t"<<"Phy RxDrop :  "<< PhyTxDropCount << "\t" <<"Phy RxDrop : "<< PhyRxDropCount <<"\t"<<"Mac RxDrop :  "<<MacRxDropCount<< "\n";
...
{   
	for(int i=0;i<2;i++)
	{  bool rtsandcts=false;
		if(i==1)
		{ 
			rtsandcts=true;
			//std::cout<<"with rts and cts";
		}

		UintegerValue ctsThr = (rtsandcts ? UintegerValue (100) : UintegerValue (2200));
		...
		ApplicationContainer sinkApps3 = packetSinkHelper3.Install (c.Get (4)); //n20 as sink
		...
		AnimationInterface anim ("9_2.xml");
		Simulator::Run ();
		...
		for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator iter = stats.begin (); iter != stats.end (); ++iter)
		{
			...
			NS_LOG_UNCOND("\nFlow ID: " << iter->first << " Sender:" << t.sourceAddress << " reciever " << t.destinationAddress);
			...
			NS_LOG_UNCOND("calculated Throughput: " << iter->second.rxBytes * 8.0 / (iter->second.timeLastRxPacket.GetSeconds()-iter->second.timeFirstTxPacket.GetSeconds()) / 1024/1024  << " Mbps");
			NS_LOG_UNCOND("Throughput percent: " << (iter->second.rxBytes * 8.0 / (iter->second.timeLastRxPacket.GetSeconds()-iter->second.timeFirstTxPacket.GetSeconds()) / 1024/1024 )/10000 <<" %" );
			// }
	}
	monitor->SerializeToXmlFile("9_2.flow.xml", true, true);
